...
# 密钥只从环境变量读取，永不写入代码/提交到 git（Step 0）。
# 原硬编码 key 已吊销：请在 Materials Project Dashboard 重新生成，再 export MP_API_KEY=...
KEY = os.environ.get("MP_API_KEY")
if not KEY:
    raise SystemExit("请设置环境变量 MP_API_KEY（登录 Materials Project → Dashboard 生成；切勿写进代码）")
